const pool = require ('../utils/pool.js');

module.exports = class Animal {
    constructor(row) {
        this.id = row.id;
        this.name = row.name;
        this.speciesId = row.species_id;
      }

      static async insert({ name, speciesId }) {

        const { rows } = await pool.query(

          'INSERT INTO animals (name, species_id) VALUES ($1, $2) RETURNING *;',
          [name, speciesId]
        );

        return new Animal(rows[0]);
      }

      static async get(id) {

        const { rows } = await pool.query('SELECT * FROM animals WHERE id=$1;', 
        [id]
        );

        return new Animal(rows[0]);
      }
    
      static async animalsAndSpecies() {

        const { rows } = await pool.query(`

        SELECT animals.name, species.type 
        FROM animals
        LEFT JOIN species
        ON animals.species_id = species.id;`

        );

        return rows;
      }

      static async put({ id, name, speciesId }) {

        const { rows } = await pool.query(

          'UPDATE animals SET name = $1, species_id =$2 WHERE id = $3 RETURNING *;',
          [name, speciesId, id]
        );

        return new Animal(rows[0]);
      }

      static async delete(id) {

        const { rows } = await pool.query(

          'DELETE FROM animals WHERE id = $1 RETURNING *;',
          [id]
        );

        return new Animal(rows[0]);
      }

      static async countBySpecies() {

        const { rows } = await pool.query(`

        SELECT species.type, COUNT(*) FROM animals
        JOIN species
        ON animals.species_id = species.id
        GROUP BY species.type
        ORDER BY count, species.type;`

        );

        return rows;
      }
    }
    
